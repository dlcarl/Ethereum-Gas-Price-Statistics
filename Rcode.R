...
title(ylab = "Hourly median threshold price (Gwei)", xlab = "Date", line = 2)
...
date <- as.POSIXct(gp_hourly$date, origin="1970-01-01")
r <- as.POSIXct(round(range(date), "month"))
plot(date, ts_gp_log, xaxt = "n", 
     xlab = "", ylab = "", type = "l")
axis(side = 1, at = seq(r[1], r[2], by = "month"), labels = F)
ticks <- seq(r[1], r[2], by = "quarter")
mtext(strftime(ticks, format = "%Y-%m"), 
      side = 1, line = 0.5, at = ticks)
title(ylab = "Log hourly median threshold price (Gwei)", xlab = "Date",
      line = 2)

#plot 6
plot(acf(ts_gp_log, lag = 7*24), xlab = "", ylab = "", main = "", ci.col = "black")
...
date <- as.POSIXct(gp_hourly$date, origin="1970-01-01")
r <- as.POSIXct(round(range(date), "month"))
plot(date[25:length(date)],diff(ts_gp_log,24), xaxt = "n", 
     xlab = "", ylab = "", type = "l")
axis(side = 1, at = seq(r[1], r[2], by = "month"), labels = F)
ticks <- seq(r[1], r[2], by = "quarter")
mtext(strftime(ticks, format = "%Y-%m"), at = ticks, side = 1, line = 0.5)
title(ylab = "Seasonal differenced threshold price (Gwei)", xlab = "Date",
      line = 2, cex.lab = 1)

#adf test
adf.test(diff(ts_gp_log,24))

#plot 8
plot(pacf(diff(ts_gp_log,24), lag = 7*24), xlab = "", ylab = "", main = "", ci.col = "black")
title(ylab = "Partial autocorrelation function (PACF)", line = 2.5)
title(xlab = "Lag in days", line = 2)

#Model estimation
fit1 <- Arima(ts_gp,order = c(1,0,0), seasonal = c(0,1,1), lambda = 0)
fit2 <- Arima(ts_gp,order = c(2,0,0), seasonal = c(0,1,1), lambda = 0)
fit3 <- Arima(ts_gp,order = c(2,0,1), seasonal = c(0,1,1), lambda = 0)
fit4 <- Arima(ts_gp,order = c(2,0,1), seasonal = c(0,1,2), lambda = 0)
fit5 <- Arima(ts_gp,order = c(2,0,1), seasonal = c(1,1,1), lambda = 0)
fit6 <- Arima(ts_gp,order = c(3,0,1), seasonal = c(0,1,1), lambda = 0)
...
#plot 9
fit <- Arima(ts_gp,order = c(2,0,1), seasonal = c(0,1,1), lambda = 0)
...
title(xlab = "Number of hours forecast ahead of time",line = 2)
...
res_mape1 <- CVts_rep(dataset = ts_gp, forecast_function = far, forecast_horizon = 24,
                      error_function = P.Error, p.order = c(2,0,1), 
                      p.seasonal = c(0,1,1), n = 500, initial = 60, seed = 1)
res_mape1_rol <- CVts_rep_rol(dataset = ts_gp, forecast_function = far, forecast_horizon = 24,
                      error_function = P.Error, p.order = c(2,0,1), 
                      p.seasonal = c(0,1,1), n = 500, initial = 60, seed = 1)
plot(c(1:24),rowMeans(abs(res_mape1)), type = "l", ylim = c(0.1,0.5), 
     xlab = "", ylab = "")
points(c(1:24), y = rowMeans(abs(res_mape1_rol)), type = "l", lty = 2)
legend("bottomright", lty = c(1:2), 
       legend = c("whole data set","only data of most recent 30 days"))
title(ylab = "Mean absolute percentage error (MAPE)", line = 2.5)
title(xlab = "Number of hours forecast ahead of time",line = 2)
...
legend("bottomright", lty = c(1:3), 
       legend = c("median hourly threshold price",
                  "first quartile hourly threshold price","third quartile hourly threshold price"))
title(ylab = "Mean absolute percentage error (MAPE)", line = 2.5)
title(xlab = "Number of hours forecast ahead of time",line = 2)
...
title(ylab = "Mining strategy", line = 2.5)
title(xlab = "Block number",line = 2)
